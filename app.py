...
# ══════════════════════════════════════════════════════════════════════════════
#  GLOBAL CSS — clean academic light theme
# ══════════════════════════════════════════════════════════════════════════════
st.markdown(
    """
<style>
...
# ══════════════════════════════════════════════════════════════════════════════
#  PLOTLY THEME + COLOUR PALETTE
#  All variables kept for consistency with prior versions (requirement 5).
# ══════════════════════════════════════════════════════════════════════════════
PLOTLY_THEME = dict(
...
# ══════════════════════════════════════════════════════════════════════════════
#  UNIVERSAL SAFETY HELPERS  (requirements 1 & 2)
#
#  _can_plot(df, *cols)
#      Returns True when df is non-empty AND every listed column has > 1 unique
#      value — the exact condition from requirement 1:
#          if not df.empty and df['column_name'].nunique() > 1
#
#  _no_data()
#      Shows the standard fallback info message from requirement 2 instead of
#      letting Plotly raise a TypeError on uniform / empty data.
# ══════════════════════════════════════════════════════════════════════════════
def _can_plot(df: pd.DataFrame, *cols: str) -> bool:
    """Return True only when df is non-empty and every col has > 1 unique value."""
...
def _no_data() -> None:
    """Requirement 2 — standard fallback shown instead of a crash."""
    st.info("📊 En attente de données plus variées pour générer ce graphique.")


# ══════════════════════════════════════════════════════════════════════════════
#  SUPABASE — credentials read from st.secrets exactly as specified
# ══════════════════════════════════════════════════════════════════════════════
...
def get_supabase_client() -> Client:
    """
    Reads secrets using the exact path required:
        st.secrets["connections"]["supabase"]["SUPABASE_URL"]
        st.secrets["connections"]["supabase"]["SUPABASE_KEY"]
    Shows a clear, actionable error if the secrets block is missing.
    """
    try:
        ...
        st.error(
            f"❌ Clé manquante dans les Secrets Streamlit : `{exc}`\n\n"
...
def save_record(client: Client, record: dict) -> tuple[bool, str]:
    ...
    try:
        ...
        return False, f"❌ Erreur lors de l'insertion : {exc}"


@st.cache_data(ttl=30)
def load_data(_client: Client) -> pd.DataFrame:
    """Fetch all rows from Supabase. Returns an empty DataFrame on any failure."""
    try:
...
TARGET      = "mood_score"
MIN_ROWS_ML = 3   # minimum rows needed by PCA / Random Forest / KMeans


def add_health_label(df: pd.DataFrame) -> pd.DataFrame:
    """Derive a binary health_status column from three thresholds."""
    df = df.copy()
...
def run_random_forest(df):
    df_lbl = add_health_label(df)
    X, y = df_lbl[FEATURES].values, df_lbl["health_status"].values
...
#  st.stop() halts execution here so no chart code runs on an empty DataFrame,
#  preventing the TypeError: 'NoneType' object is not subscriptable crash.
# ══════════════════════════════════════════════════════════════════════════════
if df_raw.empty:
    st.warning(
...
# ══════════════════════════════════════════════════════════════════════════════
#  FIVE-TAB STRUCTURE  (requirement 5 — kept identical to original)
# ══════════════════════════════════════════════════════════════════════════════
...
#  TAB 1 — APERÇU
#
#  Every px.histogram is guarded by:
#      if not df.empty and df['column_name'].nunique() > 1   (requirement 1)
#  The nbins parameter is OMITTED from all histograms so Plotly auto-scales
#  for any dataset size, including very small ones.             (requirement 4)
#  If the check fails: _no_data() shows the fallback message.  (requirement 2)
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    st.markdown(
        '<div class="section-title">Distribution des variables</div>',
        unsafe_allow_html=True,
    )

    col1, col2 = st.columns(2)

    # ── Histogram: Score d'Humeur ─────────────────────────────────────────────
    with col1:
        if not df.empty and df["mood_score"].nunique() > 1:
            fig = px.histogram(
                df,
                x="mood_score",
                # nbins intentionally omitted — Plotly auto-bins (requirement 4)
    ...
    # ── Scatter Matrix (needs MIN_ROWS_ML rows and variety in all columns) ─────
...
#  TAB 2 — RÉGRESSION
#
#  Every px.scatter / go.Scatter is guarded by:
#      if not df.empty and df['x_col'].nunique() > 1 and df['y_col'].nunique() > 1
# ══════════════════════════════════════════════════════════════════════════════
with tab2:
    if len(df) < 2:
        ...
        if _can_plot(df, *FEATURES, TARGET):
            ...
            if len(np.unique(residuals)) > 1:
                ...
                st.markdown(
                    '<div class="info-box">📌 <b>Résidus :</b> Une dispersion aléatoire '
                    "autour de zéro confirme la bonne spécification du modèle et le "
                    "respect des hypothèses de la régression linéaire.</div>",
                    unsafe_allow_html=True,
                )
            else:
                _no_data()
...
#  TAB 3 — ACP / PCA
#
#  Both the PCA scatter and the biplot are guarded by:
...
with tab4:
    ...
    if len(df) < MIN_ROWS_ML:
        ...
        # ── Decision boundary scatter (PCA projection) ────────────────────────
        st.markdown(
...
with tab5:
    ...
    if len(df) < MIN_ROWS_ML:
        ...
        with col1:
            if (
                not df_cluster.empty
                and df_cluster["sleep_hours"].nunique() > 1
                and df_cluster["mood_score"].nunique() > 1
            ):
                fig7 = px.scatter(
                    df_cluster,
                    x="sleep_hours",
        ...
        with col2:
            if (
                not df_cluster.empty
                and df_cluster["stress"].nunique() > 1
                and df_cluster["social_interaction"].nunique() > 1
            ):
                fig8 = px.scatter(
                    df_cluster,
                    x="stress",
